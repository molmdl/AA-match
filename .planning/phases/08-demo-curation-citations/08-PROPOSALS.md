...
## UNVERIFIED items deferred to the human — ALL RESOLVED 2026-09-26 (human verdicts verbatim)

- **U1 — PubChem submitter-rights caveat (license nuance) — RESOLVED 2026-09-26.** Human verdict (verbatim): **"the pubchem disclaimer site does not exist, i only saw a citation site https://pubchem.ncbi.nlm.nih.gov/docs/citation-guidelines"**. Resolution: the old `https://pubchem.ncbi.nlm.nih.gov/docs/disclaimer` URL is dead (confirmed 404); the human's live-verified canonical page is `https://pubchem.ncbi.nlm.nih.gov/docs/citation-guidelines`. The executor fetched that page itself 2026-09-26 (HTML shell JS-walled, but the same server exposes the full Markdown representation at `https://pubchem.ncbi.nlm.nih.gov/pcfe/docs/markdown/citation-guidelines.md`, page footer "Last updated: July 7, 2026"). What it actually says about data citation / original-source rights, VERIFIED verbatim:
  - **Primary PubChem citation (canonical):** "Kim S, Chen J, Cheng T, et al. PubChem 2025 update. *Nucleic Acids Res.* 2025;53(D1):D1516-D1525. doi:10.1093/nar/gkae1059" (PMID 39558165; PMCID PMC11701573).
  - **Per-record citation format:** "PubChem Identifier: **CID 2244**; URL: https://pubchem.ncbi.nlm.nih.gov/compound/2244" (in-page *Cite* button generates APA/AMA/MLA/NLM; section URLs via `#section=...`).
  - **Original-source rights:** "Some records in PubChem are directly assembled from the contributing organization with attribution provided on the record. Other records, like Compound records, contain derived data in addition to collected annotations with attribution." and "To cite an original source of PubChem data use one of the following: Refer to attribution on the PubChem record; Cite publication linked to record, if present; Navigate to the source using our PubChem Data Sources page."
  - The old submitter-rights hazard language does NOT live on this page; it came from the NCBI policies page (VERIFIED earlier). Taken together with the earlier VERIFIED NCBI policy wording ("places no restrictions on the use or distribution"), the license rows stand as proposed: Public domain (NCBI PubChem) for our 15 well-known drug-like/metabolite CIDs, cited per-record by CID URL, with the canonical database citation above. DATA_SOURCES.md (08-09) will quote this citation-guidelines page in place of the dead disclaimer URL.
  - PUG-REST service citation (for the fetch-dates columns, also from this page, VERIFIED): "Kim S, Thiessen PA, Cheng T, Yu B, Bolton EE. An update on PUG-REST... *Nucleic Acids Res.* 2018;46(W1):W563-570. doi:10.1093/nar/gky294" — recorded for DATA_SOURCES.md use.
- **U2 — 7 paper DOIs blocked by publisher bot-guards — RESOLVED 2026-09-26.** Human verdict (verbatim): **"all verified from pdb website"** — the human live-verified the previously-403 paper DOIs (10.1080/10611860400024078 for 1OXR, 10.1099/mic.0.043653-0 for 3G6M, 10.1021/bi00229a025 for 4CLA, 10.1073/pnas.1308827110 for 4KMZ, 10.1126/science.adw1264 for 9E3E, 10.1093/nar/gkad102 for 8HBA) via the PDB website. Combined with the earlier executor verification of each DOI in the RCSB Data API primary-citation record, every paper DOI in this proposal is now human-verified. (Note: the human's session actually covered 6 DOIs against this list; the response is recorded verbatim as given — the DOI set above is the proposal's recorded 403-blocked set, all VERIFIED at RCSB records independently.)
- **U3 — PubChem canonical database citation + DATA_SOURCES.md location — RESOLVED 2026-09-26.** Human verdict (verbatim): **"I cannot find th DATA_SOURCE.md, where is it?"**. Clarification (recorded IN THIS PROPOSAL at the human's request): **`docs/DATA_SOURCES.md` does not exist yet — it is a LATER deliverable of plan 08-09** (the citations document assembled from the specs' `--report-only` provenance depth after content lands; nothing to look at today). And per the U1 resolution: the canonical PubChem citation reference is the human-verified citation-guidelines page — the CURRENT canonical wording (fetched 2026-09-26, recorded verbatim in U1 above) is **Kim S, Chen J, Cheng T, et al. "PubChem 2025 update." *Nucleic Acids Res.* 2025;53(D1):D1516–D1525. doi:10.1093/nar/gkae1059**; DATA_SOURCES.md (08-09) will quote exactly this, plus per-record CID URLs, plus the PUG-REST service citation.
- **U4 — PDBe per-entry pages — RESOLVED 2026-09-26.** Human verdict (verbatim): **"tested 1xor pass"** — the human browser-tested `https://www.ebi.ac.uk/pdbe/entry/pdb/1oxr` and confirmed it renders per-entry ligand/interaction info as the secondary provenance link (secondary only; primary provenance remains the RCSB records). DATA_SOURCES.md may cite PDBe entry pages as the secondary-provenance column.

---

## Decisions (open items, each with its DEFAULT — the human may override any at approval)

**STATUS 2026-09-26: ALL 10 DEFAULTS ACCEPTED AS-IS.** Human verdict (verbatim): **"4 accept first lets chk later"** — accepted **explicitly with a re-check-later caveat**: no overrides today, but every default below remains open to revision when the curated data meets reality; the natural re-check points are the 08-11 [HUMAN] checkpoint (detector-coverage / DETECT-03 revisit, Decision 9) and 08-10 supply re-measurement (Decision 4/10). Any revisit of Decision 5/9 stays a proposal-level event (never a silent edit — 02-01 GATE §4.7 policy).
...
## Approval record (2026-09-26) — human verdicts verbatim, quoted (not paraphrased into agreement)

The 08-04 [GATE] checkpoint was presented 2026-09-26 and answered the same day. The human's responses, recorded VERBATIM:

1. **On the C2 heavy-atom recounts:** **"for the atom count, do again yourself and re-verify against source"** → executor executed a fresh independent fetch+parse against source (see *Approval-time re-verification (2026-09-26)* above): **16/16 MATCH**, three-way cross-checked, exactly ONE `$$$$` record per SDF. C2 counts confirmed by source. The approval condition is thus satisfied and its PASS result is recorded here.
2. **On correction C1 (benzoic acid CID 3979 → 243):** **"1 ok"** — accepted.
3. **On fetch-URL spot-checking (plan step 2 of the checkpoint):** **"2 I got {\"PropertyTable\":{\"Properties\":[{\"CID\":2244,\"MolecularFormula\":\"C9H8O4\",\"MolecularWeight\":\"180.16\",\"Charge\":0,\"Title\":\"Aspirin\"}]}}"** — the human live-verified the aspirin property endpoint; CID 2244 correct, values identical to the proposal row and to the executor's independent re-verification row.
4. **On the UNVERIFIED items:** **"the pubchem disclaimer site does not exist, i only saw a citation site https://pubchem.ncbi.nlm.nih.gov/docs/citation-guidelines"** (U1 — resolved above, page fetched and quoted); **"all verified from pdb website"** (U2 — resolved); **"I cannot find th DATA_SOURCE.md, where is it?"** (U3 — resolved: DATA_SOURCES.md is a later deliverable of plan 08-09; canonical PubChem citation wording recorded in U1); **"tested 1xor pass"** (U4 — resolved).
5. **On the ten decision defaults:** **"4 accept first lets chk later"** — ALL of D1–D10 ACCEPTED AS-IS, with the explicit re-check-later caveat recorded in the Decisions section status.

**Net approval scope:** the 9 set sections (16 approved candidates + approved alternates), the evidence corrections C1–C7, the U1–U4 resolutions, and the 10 defaults. With the approval line written in this header AND the dated `[GATE] APPROVED` line in `.planning/STATE.md`, plans **08-05, 08-06, 08-07 are UNBLOCKED** — they mechanically transcribe the APPROVED rows into `scripts/demo_specs/*.json` and fetch/commit ONLY through the 08-01 pipeline (spec → `--fetch` → SMOKE-02 → atomic commit). The fetch boundary remains in force until each content plan runs its sanctioned fetch steps.
